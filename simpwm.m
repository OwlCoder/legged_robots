...
tgam3 = Theta00*gamma^(1/3);
y0 = [tgam3+Theta10*gamma;
      alpha*tgam3+(alpha*Theta10+c1)*gamma;
      2*(tgam3+Theta10*gamma);
      (alpha*tgam3+(alpha*Theta10+c1)*gamma)*(1-cos(2*(tgam3+Theta10*gamma)))];
...
for i=1:steps
   [tout,yout] = ode45(@(t,y)f(t,y,gamma),h,y0,opts); % Integrate for one stride
